		...
		if (ReactionRules.Contains(currentRule))
		{
			...
			if (StatsModifications.Num() > 0)
			{
				for (FStatsModifications statsMod : StatsModifications)
				{
					...
					*/
					StatComponent->ModifyStat(GetOwner(), statsMod.Stat, statsMod.ModificationValue*multiplier, statsMod.ChangeType, statsMod.ValueType, this->GetActorLocation(), WasModified, delta, newValue, ModifiedStat, statsMod.ClearChange, statsMod.AdditionInfoTags);
